...
This object is the recommended way to distinguish MicroPython from other Python
implementations (note that it still may not exist in the very minimal ports).
...
Difference to CPython:
	...
	CPython mandates more attributes for this object, but the actual useful bare
	minimum is implemented in MicroPython.
'''

maxsize = ...
'''
Maximum value which a native integer type can hold on the current platform, or
maximum value representable by MicroPython integer type, if it’s smaller than
platform max value (that is the case for MicroPython ports without long int
support).

This attribute is useful for detecting "bitness" of a platform (32-bit vs 64-bit
, etc.).
...
tracebacklimit = ...
'''
A mutable attribute holding an integer value which is the maximum number of
traceback entries to store in an exception.
...
def atexit(func: function | None):
	...
	The atexit function will return the previous value set by this function,
...
def print_exception(exc, file=stdout, /):
	'''
	Print exception with a traceback to a file-like object `file` (or `sys.stdout`
	by default).
	...
	Difference to CPython:
		...
		Unlike `traceback.print_exception()`, this function takes just exception
		value instead of exception type, exception value, and traceback object;
		file argument should be positional; further arguments are not supported.
...
def settrace(tracefunc):
	...
	present in pre-built firmware (due to it affecting performance).
