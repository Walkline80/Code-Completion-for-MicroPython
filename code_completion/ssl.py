...
def wrap_socket(sock, server_side: bool = False, key=None, cert=None,
	...
	The implementation of this function is to first create an `SSLContext` and then call the `SSLContext.wrap_socket` method on that context object.

	The arguments `sock`, `server_side` and `server_hostname` are passed through unchanged to the method call.
	...
	- `cadata` is a bytes object containing the CA certificate chain (in DER format) that will validate the peer’s certificate.
...
class SSLContext(object):
	...
	def wrap_socket(self, sock, *, server_side: bool = False,
		...
		'''
		Takes a `stream sock` (usually `socket.socket` instance of `SOCK_STREAM` type), and returns an instance of `ssl.SSLSocket`, wrapping the underlying stream.

		The returned object has the usual `stream` interface methods like `read()`, `write()`, etc.

		- `server_side` selects whether the wrapped socket is on the server or client side.

			A server-side SSL socket should be created from a normal socket returned from `accept()` on a non-SSL listening server socket.

		- `do_handshake_on_connect` determines whether the handshake is done as part of the `wrap_socket` or whether it is deferred to be done as part of the initial reads or writes For blocking sockets doing the handshake immediately is standard.
