...
def gmtime(secs: tuple = None):
	'''
	Convert the time `secs` expressed in seconds since the Epoch into an 8-tuple which contains:
...
def sleep_ms(ms):
	...
	This function will delay for at least the given number of milliseconds, but may take longer than that if other processing must take place, for example interrupt handlers or other threads.
...
def ticks_ms():
	...
	Note that only non-negative values are used. For the most part, you should treat values returned by these functions as opaque.

	The only operations available for them are `ticks_diff()` and `ticks_add()` functions described below.

	Note:

		Performing standard mathematical operations (+, -) or relational operators (<, <=, >, >=) directly on these value will lead to invalid result.
...
def ticks_add(ticks, delta):
	...
	Given a `ticks` value, this function allows to calculate ticks value `delta` ticks before or after it, following modular-arithmetic definition of tick values.
	...
	`ticks_add()` is useful for calculating deadlines for events/tasks. (Note: you must use `ticks_diff()` function to work with deadlines.)
	'''
...
def time() -> int:
	...
	If relative times are acceptable then use the `ticks_ms()` and `ticks_us()` functions.
