...
def wake_on_ulp(wake: bool):
	'''
	Configure whether or not the Ultra-Low-Power co-processor can wake the device from sleep.

	`wake` should be a boolean value.
	'''

def wake_on_ext0(pin: int | None, level: int):
	'''
...
def gpio_deep_sleep_hold(enable: bool):
	'''
	Configure whether non-RTC GPIO pin configuration is retained during deep-sleep mode for held pads.
...
def idf_heap_info(capabilities):
	...
	One of them contains the MicroPython heap and the others are used by ESP-IDF, e.g., for network buffers and other data.

	This data is useful to get a sense of how much memory is available to ESP-IDF and the networking stack in particular.

	It may shed some light on situations where ESP-IDF operations fail due to allocation failures.
	...
	The return value is a list of 4-tuples, where each 4-tuple corresponds to one heap and contains:
...
class Partition(object):
	...
	def __init__(self, id: str | int, block_size: int = 4096, /):
		...
		`id` can be a string which is the label of the partition to retrieve, or one of the constants: BOOT or RUNNING.

		`block_size` specifies the byte size of an individual block.
		'''

	@classmethod
	def find(cls, type: int = TYPE_APP, subtype: int = 0xff, label: str = None, block_size: int = 4096):
		'''
	...
	def set_boot(self):
		...
		Note:
			...
			Do not enter deepsleep after changing the OTA boot partition, without first performing a hard reset or power cycle.
	...
	def mark_app_valid_cancel_rollback(cls):
		...
		Calling mark_app_valid_cancel_rollback is required on the first boot of a new partition to avoid an automatic rollback at the next boot. This uses the ESP-IDF "app rollback" feature with "CONFIG_BOOTLOADER_APP_ROLLBACK_ENABLE" and an OSError(-261) is raised if called on firmware that doesn’t have the feature enabled. It is OK to call mark_app_valid_cancel_rollback on every boot and it is not necessary when booting firmware that was loaded using esptool.
		'''
...
class RMT(object):
	'''
	The RMT (Remote Control) module, specific to the ESP32, was originally designed to send and receive infrared remote control signals.

	However, due to a flexible design and very accurate (as low as 12.5ns) pulse generation, it can also be used to transmit or receive many other types of digital signals.
	'''
	# Constants
	PULSE_MAX = ...

	def __init__(self, channel: int, *, pin=None, clock_div: int = 8, idle_level: bool = False, tx_carrier=None):
		'''
	...
	def source_freq(cls):
		...
		Currently the source clock is not configurable so this will always return 80MHz.
		'''
	...
	def wait_done(self, *, timeout: int = 0):
		'''
		Returns True if the channel is idle or False if a sequence of pulses started with RMT.write_pulses is being transmitted.

		If the timeout keyword argument is given then block for up to this many milliseconds for transmission to complete.
		'''
	...
	def loop(self, enable_loop: bool):
		...
		`enable_loop` is bool, set to True to enable looping on the next call to RMT.write_pulses.

		If called with False while a looping sequence is currently being transmitted then the current loop iteration will be completed and then transmission will stop.
		'''
	...
	def write_pulses(self, duration, data):
		...
		Output levels are any value that can be converted to a boolean, with True representing high voltage and False representing low.
		...
		Further calls to this method will block until the end of the current loop iteration before immediately beginning to loop the new sequence of pulses.

		Looping sequences longer than 126 pulses is not supported by the hardware.
		'''

	@staticmethod
	def bitstream_channel(value:None) -> int:
		'''
...
class NVS(object):
	...
	The NVS is partitioned into namespaces and each namespace contains typed key-value pairs.

	The keys are strings and the values may be various integer types, strings, and binary blobs.
	...
	def set_blob(self, key, value):
		...
		The `value` passed in must support the buffer protocol, e.g. bytes, bytearray, str.

		(Note that esp-idf distinguishes blobs and strings, this method always writes a blob even if a string is passed in as value.)
	...
	def get_blob(self, key, buffer):
		...
		Raises an `OSError` if the key does not exist, has a different type, or if the buffer is too small.
		'''
