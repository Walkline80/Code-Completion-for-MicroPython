...
class FrameBuffer(object):
	...
	def __init__(self, buffer, width: int, height: int, format: int, stride: int, /):
		...
		- `format` specifies the type of pixel used in the FrameBuffer;
			...
			These set the number of bits used to encode a color value and the layout of these bits in buffer.

			Where a color value c is passed to a method, c is a small integer with an encoding that is dependent on the format of the FrameBuffer.

		- `stride` is the number of pixels between each horizontal line of pixels in the FrameBuffer.

			This defaults to width but may need adjustments when implementing a FrameBuffer within another larger FrameBuffer or screen.

			The buffer size must accommodate an increased step size.

		One must specify valid `buffer`, `width`, `height`, `format` and optionally `stride`.
	...
	def hline(self, x: int, y: int, w: int, c):
		...
		The line method draws the line up to a second set of coordinates whereas the hline and vline methods draw horizontal and vertical lines respectively up to a given length.
		'''
	...
	def blit(self, fbuf, x: int, y: int, key: int = -1, palette=None):
		...
		If `key` is specified then it should be a color integer and the corresponding color will be considered transparent: all pixels with that color value will not be drawn.

		If the `palette` is specified then the `key` is compared to the value from `palette`, not to the value directly from `fbuf`.
